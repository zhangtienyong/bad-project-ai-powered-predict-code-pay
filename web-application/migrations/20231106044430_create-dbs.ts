...
export async function up(knex: Knex): Promise<void> {
  await knex.schema.createTable("users", (table) => {
    ...
    table.string("role"); // employer, developer
    // table.enum("role", ["employer", "developer"]).notNullable();
  ...
  await knex.schema.createTable("developer_data", (table) => {
    ...
    table.integer("years_of_coding").notNullable(); //.defaultTo(1);
  ...
  await knex.schema.createTable("company", (table) => {
    ...
    table.string("company_size").notNullable(); // enum / reference table
  ...
  await knex.schema.createTable("jobs", (table) => {
    ...
    table.string("employment_type").notNullable(); // enum / ref
