...
def transform_json_input(json_input):
    ...
    return {"Country": country, "EdLevel": ed_level, "YearsCodePro": years_code_pro}
...
async def predict_salary(request):
    try:
        json_input_from_client = request.json  # req.body -> JSON
        input_data = transform_json_input(json_input_from_client)

        # Create a DataFrame with feature names and values
        input_df = pd.DataFrame([input_data])  # numpy array or dataframe
...
def transform_recommendation_input(json_input):
    ...
    user_combined_df = pd.concat([loaded_user_combined_df, user_df]).reset_index(
        drop=True
    )
    ...
    tfidf_matrix_user = loaded_tfidf_vectorizer.transform(
        user_combined_df["AggregatedSkills"]
    )
    ...
    user_skills_dict = {
        category: set(str(user_combined_df.loc[user_index, category]).lower().split())
        for category in categories
    }
    for category in categories:
    ...
    top_recommended_skills = {
        category: list(recommended_skills_dict[category])[:3] for category in categories
    }
